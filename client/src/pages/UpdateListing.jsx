...
export default function UpdateListing() {
  ...
  const listingId = params.listingId || params.id || null;
  ...
  useEffect(() => {
    ...
    const fetchListing = async () => {
      ...
      try {
        ...
        setFormData((prev) => ({
          ...prev,
          imageUrls: listing.imageUrls || prev.imageUrls,
          name: listing.name || prev.name,
          description: listing.description || prev.description,
          address: listing.address || prev.address,
          type: listing.type || prev.type,
          bedRooms: listing.bedRooms ?? prev.bedRooms,
          bathRooms: listing.bathRooms ?? prev.bathRooms,
          regularPrice: listing.regularPrice ?? prev.regularPrice,
          discountPrice: listing.discountPrice ?? prev.discountPrice,
          offer: listing.offer ?? prev.offer,
          parking: listing.parking ?? prev.parking,
          furnished: listing.furnished ?? prev.furnished,
        }));
      } catch (err) {
  ...
  const storeImage = (file) => {
    ...
    if (file.size > 2 * 1024 * 1024) return Promise.reject(new Error("File too large"));
  ...
  const handleChange = (e) => {
    ...
    // mutually exclusive type selection
    if (id === "sale" || id === "rent") {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const newId = resolveListingId(data) || data._id || (data.listing && data.listing._id);
  ...
  if (fetching) return <div className="p-6 text-center">Loading listing...</div>;
  if (error && !formData.name) return <div className="p-6 text-center text-red-600">{error}</div>;

  return (
    <main className="p-6 md:p-10 max-w-6xl mx-auto bg-gray-50 rounded-lg shadow-lg mt-5">
      <h1 className="text-3xl font-bold text-center text-gray-800 mb-10">Update a Listing</h1>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col lg:flex-row gap-8">
        ...
        <div className="flex flex-col flex-1 gap-6 bg-white p-6 rounded-lg shadow-sm">
          ...
          <div className="flex flex-col sm:flex-row gap-4 items-start">
            ...
            />
            <button type="button" disabled={uploading} onClick={handleImageSubmit} className="p-3 text-green-700 border border-green-700 rounded-lg uppercase disabled:opacity-70">
          ...
          <button type="submit" disabled={saving || uploading} className="p-4 mt-4 bg-blue-700 text-white rounded-lg uppercase disabled:opacity-70">
